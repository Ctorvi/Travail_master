...
for i in mf_list:
    ...
    # manifs[i].plot(ax=ax2, markersize=0, lw=0.7,labels=[None,None] if i!=0 else ['stable MF','unstable MF'])
     if i=='mf_1T':
          ...
          manifs[i].plot_filled_lobe(ax=ax2, lobe_number=21,which_section=2,alpha=0.4,color='red')
          manifs[i].plot_filled_lobe(ax=ax2, lobe_number=13,which_section=2,alpha=0.4,color='blue')
          fig, ax2=manifs[i].plot_clinics(ax=ax2,label='homoclinics')
...
plt.savefig(f"{repository_path}figures/JF_75979_turnstile_v2.png",  bbox_inches="tight",dpi=720, pad_inches=0, facecolor=fig.get_facecolor())
plt.show()
