...
pert_mat_file = 'JF_77021_105_BO78_OS.mat'

patch_mat_file = './script/jellyfisch/77021/1_05/JF_patch_77021_105_HeI_667.mat'
...
tpc= tomo(f"{patch_mat_file}",ax,emi_vmin=0, emi_vmax=2.8e19)
...
for i in mf_list:
     ...
     manifs[i].plot(stepsize_limit=0.05, ax=ax, markersize=0, lw=0.5,colors=["xkcd:royal blue", "xkcd:red"],labels=['stable MF','unstable MF'] if i=='mf_1T' else [None,None])
...
ax.set_ylabel(r"$Z[m]$")
ax.set_aspect('equal') 
ax.set_title('(77021 / 1.05 s/ 1.9 rad)')
#plt.savefig('./Script/Jellyfisch/77021/1_05/backoff/figures/JF_77021_OnlySaddle.png', bbox_inches='tight', dpi=720)
plt.show()
