...
for i in range(len(repository_path)):
    ...
    list_ax[3*i].scatter(Hits[:,:, 0], Hits[:,:, 1], marker='o',color="xkcd:dark grey", s=0.6, linewidths=0)
    ...
    for mf_name in name_mf:
        if i != 3 or (i==3 and mf_name in allowed_mf_3):
         ...
         manifold.plot(stepsize_limit=0.05, ax=list_ax[3*i], markersize=0, lw=0.5,colors=["rosybrown", "xkcd:red"], labels=['stable MF','unstable MF'] if (mf_name=='mf_1T.pkl' and i==3) else [None ,None])
         manifold.plot(stepsize_limit=0.05, ax=list_ax[3*i+1], markersize=0, lw=0.5,colors=["rosybrown", "xkcd:red"])
...
list_ax[9].legend(loc='upper center', bbox_to_anchor=(0.41, 0.99), ncol=2, fontsize=7)#framealpha=0.6)
...
for i in range(2):
    top_o.plot(ax=list_ax[12+i], marker='o', color=colors[i],zorder=10,label='O-point' if i==1 else None)
    x_point1.plot(ax=list_ax[12+i], marker='x', color=colors[i],zorder=10,label='X-point' if i==1 else None)
    manifold_1T.plot(stepsize_limit=0.05, ax=list_ax[12+i], markersize=0, lw=0.5,colors=["rosybrown", "xkcd:red"])
    manifold_1B.plot(stepsize_limit=0.05, ax=list_ax[12+i], markersize=0, lw=0.5,colors=["rosybrown", "xkcd:red"])



Hits=np.load('./script/standard/ST_80064_pp_hits.npy')

list_ax[12].scatter(Hits[:,:, 0], Hits[:,:, 1],marker='o', color="xkcd:dark grey", s=0.6, linewidths=0)
...
plt.savefig("./figures/MANTIS_MF_new_v2.png", dpi=150, bbox_inches="tight", pad_inches=0, facecolor=fig.get_facecolor())
plt.show()
